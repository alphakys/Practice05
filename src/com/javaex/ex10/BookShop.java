    ...
    private static void displayBookInfo(Book[] books) {
    	...
    	for(int i=0;i<books.length;i++) {
    		if(books[i].stateCode==0) {
    			System.out.println((i+1)+" 책 제목:"+books[i].getTitle()+", 작가:"+books[i].getAuthor()+", 대여 유무: 재고없음");
    		}
    		else {
    			System.out.println((i+1)+" 책 제목:"+books[i].getTitle()+", 작가:"+books[i].getAuthor()+", 대여 유무: 재고있음");
    		}
    	}
